...
def prepareSpectra_Module(config, cube):
    ...
    if (
        ...
        and ((os.path.isfile(outputPrefix + "_all_spectra.hdf5") == True) or
             (config["GAS"]["LEVEL"] != "SPAXEL")) 
        and os.path.isfile(outputPrefix + "_bin_spectra.hdf5") == True
        and os.path.isfile(outputPrefix + "_bin_spectra_linear.hdf5") == True
    ):
        logging.info(
